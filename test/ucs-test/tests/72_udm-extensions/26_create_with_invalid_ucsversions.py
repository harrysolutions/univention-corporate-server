...
def test_create_with_invalid_ucsversions(udm, extension_type):
	...
	for (version_start, version_end) in (
		...
		with pytest.raises(udm_test.UCSTestUDM_CreateUDMObjectFailed, message=('Extension %s has been created with invalid UCS version (ucsversionstart=%r ucsversionend=%r)' % (extension_type, version_start, version_end))):
			udm.create_object(
