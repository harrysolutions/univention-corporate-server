...
title = _('Check local LDAP database for group membership attribute errors')
...
def run(_umc_instance, rerun=False, fix_log=''):
	...
	(success, output) = util.run_with_output(cmd)
	if not success:
		error = _('`/usr/share/univention-directory-manager-tools/proof_uniqueMembers -c` found an error with the local LDAP database group membership attributes.')
	...
	if rerun:
		fixed = _('`/usr/share/univention-directory-manager-tools/proof_uniqueMembers -c` found no errors with the local LDAP database group membership attributes.')
